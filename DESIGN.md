...
- Don't trust a card's own width/width_tablet/width_mobile setting to produce a multi-column grid — verify with an actual rendered screenshot, not just a settings/data check, since a non-functional control here fails completely silently. This applies at every column count, not just 3+: 2-column 48%/48% splits (About capabilities, service-page Problem/Solution and Deliverables/Results panels) hit the identical bug and need the same `grid-2up` treatment — when auditing for this bug, check the full 10-90% width range, not just the 3+-column range. It also applies when NO width was ever set at all (chip-style rows with no per-item width control set): every individual service page's own "Tools Used" chip row hit this — check both "has an explicit width in the wrong range" and "has no explicit width set" as two distinct symptoms of the same root cause, and re-scan the whole site (not just the page just edited) whenever this bug class is found once, since it tends to repeat identically across every page built from the same template pattern.
